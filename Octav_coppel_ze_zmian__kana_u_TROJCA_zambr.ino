...
#define LED2 2
#define LED3 3
#define CHANNEL_A 1 // define MIDI channel for man.I
#define CHANNEL_B 2 // define MIDI channel for man.II
#define CHANNEL_C 4 // define MIDI channel for ped.
unsigned long BAS_channel_1 = 0; // Flags for MIDI-channel A
unsigned long TREBL_channel_1 = 0;

unsigned long BAS_channel_2 = 0; // Flags for MIDI-channel B
unsigned long TREBL_channel_2 = 0;

unsigned long BAS_channel_3 = 0; // Flags for MIDI-channel C
...
// onsługa sub_I
  ...
  if (digitalRead(II_to_I) == LOW && channel == CHANNEL_A) {
    ...
    channel = channel + (CHANNEL_B - CHANNEL_A);
    MIDI.sendNoteOn (pitch, velocity, channel);
    channel = channel - (CHANNEL_B - CHANNEL_A);
  }
  ...
  if (digitalRead(super_II_to_I) == LOW && channel == CHANNEL_A){
   ...
   if (digitalRead(sub_II_to_I) == LOW && channel == CHANNEL_A) {
    ...
    channel = channel - (CHANNEL_C - CHANNEL_B);
    MIDI.sendNoteOn (pitch, velocity, channel);
    channel = channel + (CHANNEL_C - CHANNEL_B);
  }
  ...
  if (digitalRead(I_to_P) == LOW && channel == CHANNEL_C) {
    ...
    channel = channel - (CHANNEL_C - CHANNEL_A);
    MIDI.sendNoteOn (pitch, velocity, channel);
    channel = channel + (CHANNEL_C - CHANNEL_A);
  }
